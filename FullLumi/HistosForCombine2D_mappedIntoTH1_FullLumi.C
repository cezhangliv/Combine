...
void HistosForCombine2D_mappedIntoTH1_FullLumi(int NTOY, TString angular_cuts) {
	...
	TString mainDir = "/eos/user/r/rpilato/Analysis_MuE/Samples_FullLumi/";
	...
	for(int iK = 0; iK < ngrid; iK++) {
		for(int iM = 0; iM < ngrid; iM++) {
			TString histo_templ = dir_templ + Form("_K%i_M%i", iK, iM);
			//nominal histograms
			...
			if((i < binThR_low || i >= binThR_high) || (j < binThL_low || j >= binThL_high)) {
				...
				for(int iK = 0; iK < ngrid; iK++) {
					for(int iM = 0; iM < ngrid; iM++) {
						...
						//nominal histograms
						hn_thmuVsthe_NLO_templ[index]->SetBinContent(i, j, 0);
						hn_thmuVsthe_NLO_templ[index]->SetBinError(i, j, 0);

						//MS
						//+1sigma histograms
						hn_thmuVsthe_NLO_templ_MSUp[index]->SetBinContent(i, j, 0);
						hn_thmuVsthe_NLO_templ_MSUp[index]->SetBinError(i, j, 0);
						//-1sigma histograms
						hn_thmuVsthe_NLO_templ_MSDown[index]->SetBinContent(i, j, 0);
						hn_thmuVsthe_NLO_templ_MSDown[index]->SetBinError(i, j, 0);
					}
				}
			}
		}
...
/*	TTree *MuESetup = (TTree*) infileData->Get("MuEsetup");
	...
	for(int i = 0; i < MuESetup->GetEntries(); i++) {
		...
		sigma0_tot += MuEparams->Wnorm;//ub (Wnorm from NLO file)
	}
	sigma0_tot /= MuESetup->GetEntries();
*/
        Double_t totalEvents = 250001456;//ntot events from NLO file
        Double_t sigma0_tot  = 1336.84;//ub (Wnorm from NLO file)

	Double_t MCLumi = totalEvents/sigma0_tot;//ub-1
	cout<<"totEvents = "<<totalEvents<<" sigma0 = "<<sigma0_tot<<" MCLumi = "<<MCLumi<<endl;
	Double_t scaleFactor_data = FullLumi/MCLumi;
...
/*	MuESetup = (TTree*) infileTemplate->Get("MuEsetup");
	...
	sigma0_tot /= MuESetup->GetEntries();
*/	
        totalEvents = 250001456;//ntot events from NLO file
        sigma0_tot  = 1336.84;//ub (Wnorm from NLO file)
	MCLumi      = totalEvents/sigma0_tot;//ub-1
	Double_t scaleFactor_templ = FullLumi/MCLumi;
...
*/
        ...
        sigma0_tot  = 1336.84;//ub (Wnorm from NLO file)
	MCLumi      = totalEvents/sigma0_tot;//ub-1
	Double_t scaleFactor_MSUp = FullLumi/MCLumi;
...
*/
        ...
        sigma0_tot  = 1336.84;//ub (Wnorm from NLO file)
	MCLumi      = totalEvents/sigma0_tot;//ub-1
	Double_t scaleFactor_MSDown = FullLumi/MCLumi;
	...
	for(int iK = 0; iK < ngrid; iK++) {
		for(int iM = 0; iM < ngrid; iM++) {
			int index = ngrid*iK + iM;
			htempl.push_back(new TH1D(Form("htempl_K%i_M%i", iK, iM), hn_thmuVsthe_NLO_templ[index]->GetTitle(), nbins_tot, 0, nbins_tot));//nominal histograms


			//MS
			htempl_MSUp.push_back(new TH1D(Form("htempl_MSUp_K%i_M%i", iK, iM), hn_thmuVsthe_NLO_templ_MSUp[index]->GetTitle(), nbins_tot, 0, nbins_tot));//+1sigma histograms
			htempl_MSDown.push_back(new TH1D(Form("htempl_MSDown_K%i_M%i", iK, iM), hn_thmuVsthe_NLO_templ_MSDown[index]->GetTitle(), nbins_tot, 0, nbins_tot));//-1sigma histograms
			...
			for(int i = 0; i < hn_thmuVsthe_NLO_ref->GetNbinsX(); i++) {
				for(int j = 0; j < hn_thmuVsthe_NLO_ref->GetNbinsY(); j++) {
					...
					Double_t Nevents = hn_thmuVsthe_NLO_templ[index]->GetBinContent(i+1, j+1);
					...
					Double_t Nevents_MSUp = hn_thmuVsthe_NLO_templ_MSUp[index]->GetBinContent(i+1, j+1);
					...
					//template histograms with MultipleScattering modelization shifted by -1sigma
					Double_t Nevents_MSDown = hn_thmuVsthe_NLO_templ_MSDown[index]->GetBinContent(i+1, j+1);
	...
	if(SAVE == 1) {
		...
		for(int iK = 0; iK < ngrid; iK++) {
			for(int iM = 0; iM < ngrid; iM++) {
				int index = ngrid*iK + iM;
				//MS
				htempl_MSUp[index]->Write(Form("muemue_template_K%i_M%i_MultipleScatteringUp", iK, iM), TObject::kOverwrite);
				htempl_MSDown[index]->Write(Form("muemue_template_K%i_M%i_MultipleScatteringDown", iK, iM), TObject::kOverwrite);
			}
		}
		outfile->Close();
	}
